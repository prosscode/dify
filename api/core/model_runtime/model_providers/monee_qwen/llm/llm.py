...
class QwenAPILargeLanguageModel(_CommonOaiApiCompat, LargeLanguageModel):
    """
        ...
        entity = AIModelEntity(
            ...
            parameter_rules=[
                ParameterRule(
                    ...
                    default=float(credentials.get("temperature", 0.3)),
                    min=0.0,
                    max=2.0,
                    precision=2,
                ),
                ...
                ParameterRule(
                    ...
                    type=ParameterType.INT,
                    default=int(credentials.get("frequency_penalty", 0)),
                    min=0,
                    max=99,
                ),
                ParameterRule(
                    name=DefaultParameterName.SEED.value,
                ...
                ParameterRule(
                    ...
                    label=I18nObject(
                        zh_Hans='回复格式', en_US='Response format'
                    ),
                    options=["text", "json_object", "json_schema"],
                ),
            ],
            pricing=PriceConfig(
                input=Decimal(credentials.get("input_price", 0.001)),
                output=Decimal(credentials.get("output_price", 0.002)),
                unit=Decimal(credentials.get("unit", 0.001)),
                currency=credentials.get("currency", "RMB"),
            ),
        )
    ...
    def _generate(
        ...
        logger.info("Generating model: %s, model_parameters %s", model, model_parameters)
